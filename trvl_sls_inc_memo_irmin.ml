...
let generate_paths g =
    ...
    (memoize IrminMem Travelling_salesman.permutations) @@ (memoize2 IrminMem List.range) 0 @@ Array.length g
...
let travelling_salesman_v graph_inc paths = Inc.map2 graph_inc paths ~f:(memoize2 IrminMem @@ fun g p -> List.fold_left 
            (fun (x,sp) p -> 
